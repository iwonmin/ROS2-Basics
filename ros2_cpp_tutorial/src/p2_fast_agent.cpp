...
class FastAgent : public rclcpp::Node {
    public:
        FastAgent() : Node("fast_agent"), count_(0) {
            publisher_ = this->create_publisher<std_msgs::msg::String>("fast_topic",10);
            timer_ = this->create_wall_timer(500ms, std::bind(&FastAgent::timer_callback, this));
            subscription_ = this->create_subscription<std_msgs::msg::String>("slow_topic", 10, std::bind(&FastAgent::topic_callback, this, _1));
        }
